...
export default function Estadisticas() {
  ...
  async function cargarEstadisticas() {
    ...
    const [{ data }, { data: dataEfectivo }] = await Promise.all([
      supabase.from("registros").select("fecha, importe, origen").gte("fecha", inicioStr).lte("fecha", finStr),
      supabase.from("efectivo_dia").select("importe").gte("fecha", inicioStr).lte("fecha", finStr),
    ])
      ...
      {loading ? <p className="text-center text-gray-400">Cargando...</p> : (
        <>
          ...
          <div className="bg-green-50 dark:bg-green-900 rounded-2xl p-4 shadow">
            ...
            <p className="text-green-700 dark:text-green-300 font-black text-3xl mt-1">{totalEfectivo.toFixed(2)} €</p>
          </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow">
            ...
            <div className="space-y-3">
              {[
                { label: "🚕 Taxímetro", valor: origenes.taximetro, color: "bg-yellow-400" },
                { label: "🔴 FreeNow", valor: origenes.freenow, color: "bg-red-500" },
                { label: "⚫ Uber", valor: origenes.uber, color: "bg-gray-800 dark:bg-gray-400" },
              ].map((o) => (
                <div key={o.label}>
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">{o.label}</span>
